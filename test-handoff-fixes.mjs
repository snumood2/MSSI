...
assert.match(signup, /if \(!\/\^\\d\{8\}\$\/\.test\(patientNumber\)\) throw "의사에게 받은 번호는 8자리 숫자로 입력하세요\.";[\s\S]*patient_number: patientNumber/);
...
assert.match(app, /if \(!\/\^\\d\{8\}\$\/\.test\(patientNumber\)\) throw "의사에게 받은 번호는 8자리 숫자로 입력하세요\.";[\s\S]*patient_number: patientNumber/);
