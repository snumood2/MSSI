...
const STATS = {
  ...
  BAPQ_Total: { pat_m: 3.320423, pat_sd: 0.628963, nor_m: 2.975079, nor_sd: 0.542255 },
  BAPQ_Aloof: { pat_m: 3.572770, pat_sd: 0.949880, nor_m: 3.223242, nor_sd: 0.775185 },
  BAPQ_Pragma:{ pat_m: 2.933685, pat_sd: 0.757842, nor_m: 2.553100, nor_sd: 0.671698 },
  BAPQ_Rigid: { pat_m: 3.454812, pat_sd: 0.760960, nor_m: 3.148894, nor_sd: 0.598844 },
...
export function calculateScores(answers) {
  ...
  const socialF3 = get('f3') || get('f3_4');
  const socialF4 = get('f4') || get('f3_4');
  const ocdObsession = (get('g1a') && get('g2') && get('g5')) ? 'O' : 'X';
  const ocdCompulsion = (get('g3a') && get('g5')) ? 'O' : 'X';
  ...
  const diag = {
    ...
    social:  (get('f1') && socialF3 && socialF4 && get('f6')) ? 'O' : 'X',
    ocdObsession,
    ocdCompulsion,
    ocd:     (ocdObsession === 'O' || ocdCompulsion === 'O') ? 'O' : 'X',
...
export function generateReport(scores, answers) {
    ...
    groups: [
      ...
      },
      {
        ...
        items: [
          ...
          { name: "강박사고",  value: scores.DIAG.ocdObsession || scores.DIAG.ocd },
          { name: "강박행동",  value: scores.DIAG.ocdCompulsion || scores.DIAG.ocd },
  ...
  sections.push({
    ...
    groups: [
      {
        description: "기분변동성 기질은 기분 변화 및 기복, 그에 따른 행동 양상이 반영된 개인의 안정적인 특성(trait)을 알아보는 검사입니다  (환자군평균 29.8, 정상군평균 21.8). \"기분기복 (환자군평균 7.6, 정상군평균 4.9)\"은 기분과 에너지, 인지 및 사회적인 행동의 변동성을 나타내는 특성이고,  \"기분하향 (환자군평균 6.8, 정상군평균 4.8)\"은 우울감 및 불안, 짜증을 느끼기 쉬운 특성이고, \"기분상승 (환자군평균 5.6, 정상군평균 4.7)\"은 긍정적이고 에너지가 넘치는 느낌에 대한 특성입니다. \"계절 기분변동성 (환자군평균 3.5, 정상군평균 3.7)\"은 계절 변화에 따라 달라지는 기분, 에너지, 행동 양상에 대한 특성이고, \"소아기 기분변동성 (환자군평균 3.0, 정상군평균 2.6)\"은 아동기에 형성된 애착 유형과 정서처리 방식이 현재 대인관계 유형에 얼마나 영향을 끼치는지 알아보는 특성입니다.",
        rows: [
...

  {
    ...
    sections.push({
      ...
      groups: [
        ...
        {
          label: "범 행동문제 척도",
          description: "범 행동문제 척도는 자폐스펙트럼장애의 경도 또는 임상적 수준 이하의 증상을 확인하기 위한 검사입니다 (환자군평균 3.32, 정상군평균 2.98). \"무심한 성격은 (환자군평균 3.57, 정상군평균 3.22)\" 사회적 상호작용에 대한 관심이나 흥미가 부족한 특성이고, \"실용적 언어사용은 (환자군평균 2.93, 정상군평균 2.55)\" 사회적 의사소통 혹은 유동적이고 상호관계적 대화에 어려움이 있는 특성이며, \"경직된 성격은 (환자군평균 3.45, 정상군평균 3.15)\" 변화에 대한 관심이 적거나 변화에 적응하는 것에 어려움이 있는 특성입니다.",
          rows: [
...
export function getGlobalInstructions() {
  return `이 검사 결과는 본인이 직접 응답하신 내용을 바탕으로 산출된 것입니다. "응답결과", "환자비교백분위", "정상군비교백분위"는 모두 응답자 본인의 결과입니다.

결과를 해석할 때는 "백분위"를 중심으로 보시면 됩니다. "환자비교백분위"는 클리닉에 방문한 환자 100명 중 본인의 점수가 몇 번째에 해당하는지를, "정상군비교백분위"는 정신건강의학과를 방문한 적이 없는 사람 100명 중 몇 번째에 해당하는지를 나타냅니다.

등수가 1에 가까울수록 해당 항목의 점수가 높고 (빨간색), 100에 가까울수록 낮습니다 (파란색). 예를 들어, 순환성기질의 환자비교백분위가 69등, 정상군비교백분위가 83등이라면, 분당서울대병원 기분장애클리닉 환자 100명 중에서는 69번째로, 정신건강의학과를 방문한 적 없는 사람 100명 중에서는 83번째로 순환성기질이 높다는 의미입니다.

점수가 높다고 항상 좋거나 나쁜 것은 아니며, 그 의미는 항목마다 다릅니다. 회복력과 정서조절능력은 점수가 높을수록 긍정적이지만, 그 외 대부분의 항목은 점수가 높을수록 부정적입니다. 과잉기질, 행동억제/행동활성, 아침/저녁형처럼 좋고 나쁨을 단정하기 어려운 항목도 있습니다. 각 항목의 구체적인 의미는 검사 설명을 참고하시고, 설명에 제시된 점수와 비교하실 때는 [응답결과] 점수를 이용하세요.`;
}
