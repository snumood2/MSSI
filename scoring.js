...
export function generateReport(scores, answers) {
  ...
  function makeSpecialRow(name, scoreVal, desc) {
    return {
      name,
      score: scoreVal,
      pat_rank: "-",
  ...
  // ── 섹션 4: 아동기외상, 대인관계민감성, 회복탄력성 ──
  {
    ...
    rows.push(makeRow("  감정적학대",          scores.CTQ.ea,     'CTQ_EA',     ""));
    rows.push(makeRow("  신체적학대",          scores.CTQ.pa,     'CTQ_PA',     ""));
    rows.push(makeRow("  성적학대",            scores.CTQ.sa,     'CTQ_SA',     ""));
    rows.push(makeRow("  감정적방치",          scores.CTQ.en,     'CTQ_EN',     ""));
    rows.push(makeRow("  신체적방치",          scores.CTQ.pn,     'CTQ_PN',     ""));
    rows.push(makeRow("대인관계민감도 총점",   scores.IPSM.total, 'IPSM_Total', ipsmDesc));
    rows.push(makeRow("  대인인식",            scores.IPSM.ia,    'IPSM_IA',    ""));
    rows.push(makeRow("  승인욕구",            scores.IPSM.na,    'IPSM_NA',    ""));
    rows.push(makeRow("  분리불안",            scores.IPSM.sa,    'IPSM_SA',    ""));
    rows.push(makeRow("  소심함",              scores.IPSM.tim,   'IPSM_TIM',   ""));
    rows.push(makeRow("  유약한내적자기",      scores.IPSM.fis,   'IPSM_FIS',   ""));
    ...
    rows.push(makeRow("  낙관성",              scores.CD.optimism,'CD_Optimism',""));
    rows.push(makeRow("  지지",               scores.CD.support, 'CD_Support', ""));
    rows.push(makeRow("  영성",               scores.CD.spirit,  'CD_Spirit',  ""));
    sections.push({ title: "아동기외상, 대인관계민감성, 회복탄력성", rows: rows.filter(Boolean), specialRows: [] });
  }
  ...
  // ── 섹션 5: 정서조절, 행동패턴, 음주, 수면양상 ──
  {
    ...
    const auditDesc = "음주 경향, 의존성, 위험 정도를 측정합니다 (환자군평균 6.9, 정상군평균 2.6). 응답결과가 12점 이상이면 약한 수준, 20점 이상이면 중등도 수준, 24점 이상이면 심한 수준의 문제가 있음을 의미합니다.";
    const cmsDesc = "아침 활동형, 저녁 활동형 여부를 측정하는 검사로 (환자군평균 28.7, 정상군평균 30.6), 점수가 높을수록 아침형, 낮을수록 저녁형에 해당합니다. 응답결과가 41점 이상은 아침형, 26점 이하는 저녁형에 해당하며, 그 중간 점수에 있을 경우에는 구분이 뚜렷하지 않은 중간형에 해당합니다.";
    const rows = [];
    rows.push(makeRow("정서조절능력 총점",   scores.ERSQ.total,    'ERSQ_Total',    ersqDesc));
    rows.push(makeRow("  의식",              scores.ERSQ.aware,    'ERSQ_Aware',    ""));
    rows.push(makeRow("  감각",              scores.ERSQ.body,     'ERSQ_Body',     ""));
    rows.push(makeRow("  명료성",            scores.ERSQ.clarity,  'ERSQ_Clarity',  ""));
    rows.push(makeRow("  이해",              scores.ERSQ.under,    'ERSQ_Under',    ""));
    rows.push(makeRow("  수용",              scores.ERSQ.accept,   'ERSQ_Accept',   ""));
    rows.push(makeRow("  회복력",            scores.ERSQ.resil,    'ERSQ_Resil',    ""));
    rows.push(makeRow("  지지",              scores.ERSQ.support,  'ERSQ_Support',  ""));
    rows.push(makeRow("  내성",              scores.ERSQ.tolerate, 'ERSQ_Tolerate', ""));
    rows.push(makeRow("  수정",              scores.ERSQ.modify,   'ERSQ_Modify',   ""));
    ...
    rows.push(makeRow("음주경향",             scores.AUDIT,         'AUDIT',         auditDesc));

    // 아침/저녁형 with extra classification
    const cmsRow = makeRow("아침/저녁형", scores.CMS.score, 'CMS', cmsDesc);
    if (cmsRow) cmsRow.extra = scores.CMS.class;
    rows.push(cmsRow);

    sections.push({ title: "정서조절, 행동패턴, 음주, 수면양상", rows: rows.filter(Boolean), specialRows: [] });
  }

  // ── 섹션 6: 계절성 우울증, 집중력, 경계선 성격, 행동문제, 성격특성 ──
  {
    const spaqDesc = "계절에 따라 수면, 기분, 식욕, 활력 등이 달라지는 경향을 보기 위한 검사입니다. 응답결과가 높을수록 계절에 따른 차이가 크다는 것을 의미합니다. 계절성이 심하고 이에 따른 일상생활의 지장이 크다면 계절성 우울증(SAD)에 해당할 수 있습니다. 설문 결과상 계절성 우울증이 약하게 있으면 \"subsyndromal SAD\" 없으면 \"not SAD\"로 표시됩니다. (환자군평균 6.0 정상군평균 5.9) 현재 판정: " + scores.SPAQ.class;
    const asrsDesc = "성인 ADHD 검사입니다 (환자군평균 46.8, 정상군평균 39.9). 6개 항목은 ADHD 진단기준에 대해 물어보는 것입니다. 이 중 4개 이상의 항목을 만족하면 성인 ADHD를 의심할 수 있습니다. 그런 경우 \"성인ADHD의심됨\"이라고 표시됩니다. 그러나 이것만으로 ADHD로 진단할 수는 없고, ADHD가 아니라 기분장애의 증상으로 집중력 문제가 있을 수도 있습니다. 하단의 점수는 ADHD 증상의 정도를 나타냅니다.";
    const wursDesc = "아동기 시절의 ADHD 증상과 병력을 측정하기 위한 검사입니다 (환자군평균 25.9, 정상군평균 19.8). 응답결과가 46점 이상일 경우, 과거 ADHD 경향을 보였을 가능성이 높을 수 있습니다.";
    ...
    // 계절성우울증
    const spaqRow = makeRow("계절성우울증", scores.SPAQ.score, 'SPAQ', spaqDesc);
    if (spaqRow) spaqRow.extra = scores.SPAQ.class;
    rows.push(spaqRow);

    // 성인기 주의집중문제: 선별결과 행 + 점수 행
    rows.push({
      name: "성인기 주의집중문제",
      score: scores.ASRS.result,
      pat_rank: "-",
      nor_rank: "-",
      pat_color: "",
      nor_color: "",
      description: asrsDesc,
      special: "adhd_screening"
    });
    rows.push(makeRow("", scores.ASRS.total, 'ASRS', ""));

    rows.push(makeRow("아동기 주의집중문제",   scores.WURS,          'WURS',       wursDesc));
    rows.push(makeRow("범 행동문제 척도",       scores.BAPQ.total,    'BAPQ_Total', bapqDesc));
    rows.push(makeRow("  무심한 성격",          scores.BAPQ.aloof,    'BAPQ_Aloof', ""));
    rows.push(makeRow("  실용적 언어사용",      scores.BAPQ.pragma,   'BAPQ_Pragma',""));
    rows.push(makeRow("  경직된 성격",          scores.BAPQ.rigid,    'BAPQ_Rigid', ""));
    rows.push(makeRow("경계성 성격장애 특성",   scores.BOR,           'BOR',        borDesc));

    sections.push({ title: "계절성 우울증, 집중력, 경계선 성격, 행동문제, 성격특성", rows: rows.filter(Boolean), specialRows: [] });
  }
  ...
  if (scores.PMS.sym > 0 || scores.PMS.func > 0) {
    ...
    rows.push(makeSpecialRow("생리증후군 여부", scores.PMS.diag, ""));
    sections.push({ title: "생리주기에 따른 변화 (여성만 해당됩니다)", rows: rows.filter(Boolean), specialRows: [] });
  }

  // ── 섹션 8: 혼합/초조 우울증 및 양극성장애 선별 ──
  {
    const rows = [];
    rows.push(makeRow("기분불안정상태검사",   scores.MIQS,          'MIQS',        ""));
    rows.push({
      name: "초조우울증선별",
      score: scores.AgitatedDep,
      pat_rank: "-", nor_rank: "-",
      pat_color: scores.AgitatedDep === "양성" ? "#FF5C7A" : "#2B3CFF",
      nor_color: "", description: ""
    });
    rows.push({
      name: "혼합우울증선별",
    ...
    sections.push({ title: "혼합/초조 우울증 및 양극성장애 선별", rows: rows.filter(Boolean), specialRows: [] });
  }
